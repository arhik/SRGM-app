...
export class Upload{
	...
	*/
	...
	csv_data(){
		...
		this.http.fetch("/upload",{method:"post",body: data}).then(response => response.json()).then(formatted_data => this.eventAggregator.publish('rawdata', JSON.parse(formatted_data)));
	...
	computedata(){
		...
		this.http.fetch("/compute",{method: "post", body: this.originaldata.getRawData()}).then(response => response.json()).then(formattedplot_data => this.eventAggregator.publish('plotData',formattedplot_data));
		this.msg = "Plot data is here"
	}

	load_sample(){
		console.log("Loading sample data ...");
		this.http.fetch("/sample",{method:"get"}).then(response => response.json()).then(formatted_data => this.eventAggregator.publish('rawdata', JSON.parse(formatted_data)));
		this.msg = "sample is loaded for you."
		console.log("Completed loading sample");
